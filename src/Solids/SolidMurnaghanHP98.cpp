...
auto thermoPropertiesMinMurnaghanEOSHP98(Reaktoro_::Temperature TK, Reaktoro_::Pressure P, Substance subst, ThermoPropertiesSubstance tps) -> ThermoPropertiesSubstance
{
...
//        aW.twp->Bet = 1./(P/1000) * (1. - pow( (1.- 4.*(P/1000)/(kap + 4.*(P/1000) )), 0.25 ));  // compressibility at T (to check)
        ...
        ds = ( - ( (Vst*kap/3.)*(a0-10.*a0*pow(TK,-0.5)) * (pow((1.+4.*(P/1000)/kap),0.75)-1.)
                   - (Vt*k0/3.)*(1.5e-4) * (pow((1.+4.*(P/1000)/kap),0.75)-1.)
                   + Vt*(P/1000)*(1.5e-4)/(1.-(1.5e-4)*(TK-Tst)) * pow((1.+4.*(P/1000)/kap),(-0.25)) ) * (1000.) );
        dh = dg + TK*ds;
        dv = ( Vt*pow( (1.- 4.*(P/1000)/(kap + 4.*(P/1000) )),0.25 ));
        dcp = - TK * ( (5./3.)*Vst*a0*kap/pow(TK,1.5) * (pow((1.+4.*(P/1000)/kap),0.75)-1.)
                    - (2./3.)*Vst*(a0-10.*a0/pow(TK,0.5))*k0*(1.5e-4) * (pow((1.+4*(P/1000)/kap),0.75)-1.)
                    + 2.*Vst*(a0-10.*a0/pow(TK,0.5))*(P/1000)*(1.5e-4) / (1.-(1.5e-4)*(TK-Tst)) * pow((1.+4.*(P/1000)/kap),(-0.25))
                    - Vt*pow((P/1000),2.)*pow((1.5e-4),2.) / ( pow((1.-(1.5e-4)*(TK-Tst)),3.)*k0 )
    ...
    subst.checkCalcMethodBounds("Holland and Powell Murnaghan model", TK.val-C_to_K, P.val, tps);
