...
#include <Solvent/Reaktoro/WaterHelmholtzStateHGK.hpp>
...
auto waterIdealGas (Reaktoro_::Temperature t, PropertiesSolvent &wp) -> void
{
    ...
    tt  = t / 1.0e2;
    tl  = log(tt);
    wp.gibbsIdealGas  = - (c[0] / tt + c[1]) * tl;
    enthalpyIdealGas  = (c[1] + c[0] * (1.0e0 - tl) / tt);
    wp.cpIdealGas = c[1] - c[0] / tt;
    while ( ++i <= 17 )
    {
        emult = pow(tt,((double)i - 5.));
        wp.gibbsIdealGas  -= c[i] * emult;
        enthalpyIdealGas  += c[i] * (i-5) * emult;
        wp.cpIdealGas += c[i] * (i-5) * (i-4) * emult;
    }
    helmholtzIdealGas  = wp.gibbsIdealGas - 1.0e0;
    internalEnergyIdealGas  = enthalpyIdealGas - 1.0e0;
    cvIdealGas = wp.cpIdealGas - 1.0e0;
    wp.entropyIdealGas  = internalEnergyIdealGas - helmholtzIdealGas;
}
